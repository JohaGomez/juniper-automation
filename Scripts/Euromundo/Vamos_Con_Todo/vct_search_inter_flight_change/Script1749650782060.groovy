...
// Selección de tarifa
TestObject cambiarTarifa = new TestObject().addProperty('xpath', ConditionType.EQUALS, "//button[contains(@class,'result-option__select-fare-button') and contains(text(),'Cambiar tarifa')]")
TestObject seleccionarVuelo = new TestObject().addProperty('xpath', ConditionType.EQUALS, "//button[contains(@class,'result-option__choose-button') and contains(text(),'Seleccionar vuelo')]")

if (WebUI.verifyElementPresent(cambiarTarifa, 5, FailureHandling.OPTIONAL)) {
    WebUI.comment("🔄 Se encontró botón 'Cambiar tarifa'.")
    WebUI.click(cambiarTarifa)
} else if (WebUI.verifyElementPresent(seleccionarVuelo, 5, FailureHandling.OPTIONAL)) {
    WebUI.comment("✈️ Se encontró botón 'Seleccionar vuelo'.")
    WebUI.click(seleccionarVuelo)
} else {
    WebUI.comment("❌ No se encontró ni 'Cambiar tarifa' ni 'Seleccionar vuelo'.")
    WebUI.markFailed("No se encontró botón de acción para continuar con el vuelo.")
}
