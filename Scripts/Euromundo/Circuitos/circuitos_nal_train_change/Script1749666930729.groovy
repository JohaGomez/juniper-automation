...
// 🏨 Verificar si aparece botón cambiar tren
boolean trenVisible = WebUI.waitForElementVisible(findTestObject('Euromundo/services/button_change_train'), 10, FailureHandling.OPTIONAL)
...
WebUI.scrollToElement(findTestObject('Euromundo/services/button_change_train'), 10)
WebUI.click(findTestObject('Euromundo/services/button_change_train'))

// Seleccionar el nuevo tren 🚄
WebUI.waitForElementClickable(findTestObject('Euromundo/circuitos/repository_circuitos_nal/button_select_new_train'), 15)
...
// Prebook
WebUI.waitForElementClickable(findTestObject('Euromundo/book_steps/button_prebook'), 15)
...
// Guardar titulo del tren nuevamente para comparar
TestObject otroTituloObj = new TestObject("otroTitulo")
otroTituloObj.addProperty("xpath", ConditionType.EQUALS, '//*[@id="main-content"]/div[2]/div/div[3]/div[1]/div/div/div/section/div/div[3]/div[1]/div/div/div[1]')

// Capturar el texto del segundo título
String tituloNuevo = WebUI.getText(otroTituloObj).trim()

// Comparar
if (tituloGuardado.equalsIgnoreCase(tituloNuevo)) {
	WebUI.comment("✅ Los títulos son iguales: " + tituloGuardado)
} else {
	WebUI.comment("❌ Los títulos son diferentes -> Guardado: " + tituloGuardado + " | Nuevo: " + tituloNuevo)
}
WebUI.scrollToElement(new TestObject().addProperty("xpath", ConditionType.EQUALS, "//*[@id='menu-option-national-hotels']"), 10)
...
WebUI.setText(findTestObject('Euromundo/transfers/select_airport_destination'), 'Bogota')

TestObject optionElDorado = new TestObject('dynamicAutocompleteOption_ElDorado')
optionElDorado.addProperty(
    "xpath",
    ConditionType.EQUALS,
    "//div[contains(@class,'tt-suggestion') and contains(.,'Aeropuerto El Dorado')]")

WebUI.waitForElementVisible(optionElDorado, 10)
WebUI.click(optionElDorado)

// 🛫Ingresar datos de vuelo
WebUI.setText(findTestObject('Euromundo/transfers/set_flight_number'), 'AV123')

// 📍Ingresar datos de punto de destino
WebUI.selectOptionByValue(findTestObject('Euromundo/transfers/select_destination_point'), '1017', true)
