...
CustomKeywords.'utils.FechaUtils.setFechaLunesEnUnMesAleatorio'('Euromundo/book_steps/origin_date_inter') // 📅 Fecha
...
boolean hotelVisible = WebUI.waitForElementVisible(findTestObject('Euromundo/services/button_change_train'), 10, FailureHandling.OPTIONAL)

WebUI.click(findTestObject('Euromundo/services/button_change_train'))

if (!(hotelVisible)) {WebUI.comment('🔄 No se cargó a la primera. Reintentando...')
    WebUI.waitForElementClickable(findTestObject('Euromundo/book_steps/button_edit'), 10)
    WebUI.click(findTestObject('Euromundo/book_steps/button_edit'))
    WebUI.click(findTestObject('Euromundo/book_steps/button_search_inter'))
    WebUI.waitForElementVisible(findTestObject('Euromundo/services/button_change_train'), 10)
	WebUI.click(findTestObject('Euromundo/services/button_change_train'))
}

// ✅ Prebook
WebUI.click(findTestObject('Euromundo/circuitos/repository_circuitos_nal/button_select_new_train'))

//Guardar titulo del tren para después comparar
TestObject tituloObj = new TestObject("tituloDinamico")
tituloObj.addProperty(
	"xpath",
	ConditionType.EQUALS,
	"//*[@id='main-content']/div/div[2]/div[1]/div/div/div/article/section[1]/div/div[3]/div[1]/div[1]/div/div[1]")
...
//Guardar titulo del tren para comparar
TestObject otroTituloObj = new TestObject("otroTitulo")
otroTituloObj.addProperty(
    "xpath",
    ConditionType.EQUALS,
    "//*[@id='main-content']/div[2]/div/div[3]/div[1]/div/div/div/section/div/div[3]/div[1]/div/div/div[1]")
...
Map<String, String> campos = [
	'Euromundo/transfers/set_hotel_origin_transfer'  : 'Hotel ITO',
	'Euromundo/transfers/set_address_origin_transfer': 'Aeropuerto',
	'Euromundo/transfers/select_airport_destination' : 'Bogota'
]

campos.each { path, valor ->
	TestObject campo = findTestObject(path)

	// Verifica si el campo existe y está visible en la página
	if (WebUI.verifyElementPresent(campo, 5, FailureHandling.OPTIONAL) &&
		WebUI.verifyElementVisible(campo, FailureHandling.OPTIONAL)) {

		try {
			WebUI.setText(campo, valor)
			WebUI.comment("✅ Campo '${path}' llenado con: ${valor}")
		} catch (Exception e) {
			WebUI.comment("⚠️ No se pudo escribir en '${path}': ${e.message}")
		}

	} else {
		WebUI.comment("ℹ️ Campo '${path}' no encontrado o no visible. Se continúa el flujo.")
	}
}

// Selección del aeropuerto (autocomplete)
TestObject optionElDorado = new TestObject('dynamicAutocompleteOption')
optionElDorado.addProperty("xpath", ConditionType.EQUALS, "//div[contains(@class,'tt-suggestion') and contains(.,'Aeropuerto El Dorado')]")

if (WebUI.waitForElementVisible(optionElDorado, 10, FailureHandling.OPTIONAL)) {
    WebUI.click(optionElDorado)
    WebUI.comment("✅ Aeropuerto 'El Dorado' seleccionado del autocompletado.")
} else {
    WebUI.comment("⚠️ No se encontró la sugerencia 'Aeropuerto El Dorado'. Se continúa el flujo.")
}

// Setear número de vuelo
if (WebUI.verifyElementPresent(findTestObject('Euromundo/transfers/set_flight_number'), 5, FailureHandling.OPTIONAL)) {
    WebUI.setText(findTestObject('Euromundo/transfers/set_flight_number'), 'AV123')
    WebUI.comment("🛫 Número de vuelo ingresado: AV123")
} else {
    WebUI.comment("⚠️ Campo de número de vuelo no encontrado.")
}

// Seleccionar punto de destino de la lista
WebUI.waitForElementClickable(findTestObject('Euromundo/transfers/select_airport_list'), 15)
WebUI.selectOptionByValue(findTestObject('Euromundo/transfers/select_airport_list'), '1017', true)

// Finalización del prebook
...
// ==========================
// 📌 Obtener título del Tren
// ==========================
TestObject tituloBookObj = new TestObject('tituloBookObj')
tituloBookObj.addProperty(
    "xpath",
    ConditionType.EQUALS,
    "//*[@id='main-content']/div[2]/div/div/div[1]/div[1]/div/div[2]/table/tbody/tr[3]/td/div/ul/ul[1]/li[2]/span[1]")
