...
WebUI.waitForElementVisible(findTestObject('Euromundo/Gran_Deal/repository_GD_Inter/menu_gd'), 10)
...
WebUI.waitForElementClickable(findTestObject('Euromundo/book_steps/select_origin_inter'), 30)
...
// 🏨 Reintento si no carga resultados visibles
TestObject resultado = findTestObject('Euromundo/Gran_Deal/repository_GD_Inter/card_resultado')
TestObject botonPrebook = findTestObject('Euromundo/book_steps/button_prebook')
TestObject botonEditar = findTestObject('Euromundo/book_steps/button_edit')
TestObject botonBuscar = findTestObject('Euromundo/Gran_Deal/repository_GD_Inter/button_search_inter')

WebDriver driver = DriverFactory.getWebDriver()

// Espera inicial de 10 segundos por algún resultado visible o botón de prebook
if (!WebUI.waitForElementVisible(botonPrebook, 10, FailureHandling.OPTIONAL)) {
    List<WebElement> resultados = driver.findElements(By.cssSelector(".result-card"))

    if (resultados.size() == 0) {
        // 🧯 No hay resultados, hacer reintento
        WebUI.comment("⚠️ No se encontraron resultados, reintentando búsqueda...")

        WebUI.waitForElementClickable(botonEditar, 10)
        WebUI.click(botonEditar)

        WebUI.waitForElementClickable(botonBuscar, 10)
        WebUI.click(botonBuscar)

        // Esperar nuevamente por los resultados
        WebUI.waitForElementVisible(botonPrebook, 10)
    } else {
        WebUI.comment("✅ Se encontraron resultados, continúa el flujo normal.")
    }
}
...
WebUI.waitForElementClickable(findTestObject('Euromundo/book_steps/button_prebook'), 20)
WebUI.click(findTestObject('Euromundo/book_steps/button_prebook'))
...
try {
    ...
    KeywordUtil.markFailed("❌ No se pudo seleccionar el pasajero responsable.")
}
