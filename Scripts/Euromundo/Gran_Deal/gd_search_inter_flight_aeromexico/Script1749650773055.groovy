...
WebUI.waitForElementVisible(findTestObject('Euromundo/book_steps/button_box'), 15, FailureHandling.OPTIONAL)
// Si no se encuentra el objeto en el repositorio, usar un XPath alternativo dinámico
TestObject botonBox = new TestObject('dynamic_button_box')
botonBox.addProperty('xpath', ConditionType.EQUALS, "//*[contains(@id,'main-modal')]//button[contains(@class,'btn') or contains(.,'Aceptar') or contains(.,'Continuar')]")

// Intento de clic con fallback
if (WebUI.verifyElementPresent(findTestObject('Euromundo/book_steps/button_box'), 5, FailureHandling.OPTIONAL)) {
	WebUI.click(findTestObject('Euromundo/book_steps/button_box'))
	println "✅ Clic exitoso en button_box del repositorio"
} else if (WebUI.verifyElementPresent(botonBox, 5, FailureHandling.OPTIONAL)) {
	WebUI.scrollToElement(botonBox, 5)
	WebUI.click(botonBox)
	println "⚙️ Clic realizado en botón alternativo detectado dinámicamente"
} else {
	WebUI.takeScreenshot()
	WebUI.comment("🚨 No se encontró el botón de confirmación. Verifica el modal o su XPath.")
	WebUI.verifyElementPresent(botonBox, 1, FailureHandling.STOP_ON_FAILURE)
}
...
CustomKeywords.'utils.FechaUtils.setFechaAleatoriaDesdeUnMesFuturo'('Euromundo/book_steps/origin_date_inter' // Fecha
    )
...
// ✈️ Seleccionar vuelo
 CustomKeywords.'utils.aerolinea_selector.seleccionarVueloAeromexico'()
