...
WebUI.click(findTestObject('Euromundo/seguros/repository_seguros_nal/select_seguros_nal'))
...
WebUI.selectOptionByValue(findTestObject('Euromundo/book_steps/input_destination_insurance'), '35163', true)
...
WebUI.waitForElementClickable(findTestObject('Euromundo/seguros/repository_seguros_nal/set_pax_adl_insurance'), 15)
...
TestObject mejorPrecioObj = new TestObject().addProperty(
	...
	"//*[@id='results-list']/div[1]/div[1]/article/div[1]/div[2]/div[1]/div/div[2]/span[1]"
)
...
// --------------------------------------------------
// 🧾 TestObjects actualizados para validación de precios
// --------------------------------------------------

// 🔹 Precio base
TestObject precioObj = new TestObject("precio")
precioObj.addProperty("xpath", ConditionType.EQUALS,
	"//div[contains(@class,'booking-breakdown__item--total-price')]//span[contains(@class,'booking-breakdown__item-price')]")

// 🔹 Comisiones
TestObject comisionObj = new TestObject("comision")
comisionObj.addProperty("xpath", ConditionType.EQUALS,
	"//div[contains(@class,'booking-breakdown__item--comission')]//span[contains(@class,'booking-breakdown__item-price')]")

// 🔹 Impuestos de comisión (opcional, si quieres validarlo también)
TestObject impuestoComisionObj = new TestObject("impuestoComision")
impuestoComisionObj.addProperty("xpath", ConditionType.EQUALS,
	"//div[contains(@class,'booking-breakdown__item--tax-comission')]//span[contains(@class,'booking-breakdown__item-price')]")

// 🔹 Precio final
TestObject precioFinalObj = new TestObject("precioFinal")
precioFinalObj.addProperty("xpath", ConditionType.EQUALS,
	"//div[contains(@class,'booking-breakdown__item--final-price')]//span[contains(@class,'booking-breakdown__item-price')]//h3")

// 🔹 Total adeudado (puede que no esté visible en todos los flujos)
TestObject totalAdeudadoObj = new TestObject("totalAdeudado")
totalAdeudadoObj.addProperty("xpath", ConditionType.CONTAINS,
	"//span[contains(@class,'agent-markup__total-due-price')]")

// --------------------------------------------------
// 💬 Mostrar valores en el log antes de validar
// --------------------------------------------------
String precio = WebUI.getText(precioObj)
String comision = WebUI.getText(comisionObj)
String precioFinal = WebUI.getText(precioFinalObj)

WebUI.comment("💵 Precio: ${precio} | 💼 Comisión: ${comision} | 🧾 Precio Final: ${precioFinal}")
...
// Si el texto termina con un guion, lo eliminamos
if (tituloBook.endsWith("-")) {
	tituloBook = tituloBook[0..-2].trim()  // elimina el último carácter y recorta espacios
	KeywordUtil.logInfo("✂️ Se eliminó el guion final del título.")
}

KeywordUtil.logInfo("📌 Título Book limpio: ${tituloBook}")
