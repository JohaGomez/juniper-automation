...
boolean hotelVisible = WebUI.waitForElementVisible(findTestObject('Euromundo/flights/button_flight_change_gd'), 15, FailureHandling.OPTIONAL)
...
if (!(hotelVisible)) {
    ...
    WebUI.click(findTestObject('Euromundo/book_steps/button_search_inter'))
    WebUI.waitForElementVisible(findTestObject('Euromundo/flights/button_flight_change_gd'), 15)
}

// ✈️ Seleccionar vuelo dinámicamente según aerolínea disponible

WebUI.click(findTestObject('Euromundo/flights/button_flight_change_auv'))
// Intentamos primero con Aerobus
TestObject filtroAerobus = findTestObject('Euromundo/flights/checkbox_filter_airline_aerobus')
TestObject filtroVolaris = findTestObject('Euromundo/flights/checkbox_filter_volaris')

if (WebUI.verifyElementPresent(filtroAerobus, 5, FailureHandling.OPTIONAL)) {
    // 🔹 Aerobus disponible
    WebUI.comment("➡️ Seleccionando vuelo Aerobus")
    WebUI.click(filtroAerobus)
...
} else if (WebUI.verifyElementPresent(filtroVolaris, 5, FailureHandling.OPTIONAL)) {
    // 🔹 Volaris disponible
    WebUI.comment("➡️ Seleccionando vuelo Volaris")
    WebUI.click(filtroVolaris)
    ...
    // ❌ Ninguna aerolínea encontrada
    WebUI.comment("⚠️ No se encontró ningún filtro de aerolínea disponible")
    KeywordUtil.markFailed("No se encontró filtro disponible para Aerobus ni Volaris")
}
