...
tituloObj.addProperty("xpath", ConditionType.EQUALS, "(//*[@id='results-list']/div[1]/div/div[1]/article/div[1]/div[2]/h2/a)")
...
// ✅ Validación dinámica del checkbox de términos y condiciones
TestObject tycCheckbox = new TestObject()
tycCheckbox.addProperty("xpath", ConditionType.EQUALS, "//input[@type='checkbox' and @name='termsAndConditions']")

if (WebUI.verifyElementPresent(tycCheckbox, 5, FailureHandling.OPTIONAL)) {
	WebElement element = WebUiCommonHelper.findWebElement(tycCheckbox, 5)
	
	// Asegurarse de que el checkbox está visible y habilitado antes de continuar
	if (element.isDisplayed() && element.isEnabled()) {
		boolean isSelected = element.isSelected()

		if (!isSelected) {
			WebUI.comment('⚠️ Checkbox "Términos y Condiciones" no está seleccionado. Dando clic.')
			WebUI.click(tycCheckbox)
		} else {
			WebUI.comment('🟢 Checkbox "Términos y Condiciones" ya está seleccionado.')
		}
	} else {
		WebUI.comment('🚫 El checkbox está oculto o deshabilitado, no se puede interactuar con él.')
	}
} else {
	WebUI.comment('❌ Checkbox "Términos y Condiciones" NO está presente en la página.')
}
