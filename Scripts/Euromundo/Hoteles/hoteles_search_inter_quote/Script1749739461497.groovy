...
boolean editVisible = WebUI.waitForElementVisible(findTestObject('Euromundo/book_steps/button_edit'), 5, FailureHandling.OPTIONAL)

if (!hotelVisible && editVisible) {
...
// Adicionar Seguro
WebUI.click(findTestObject('Euromundo/checkout_page/select_insurance'))

// ✅ Confirmación: breakdown tarifa
WebUI.click(findTestObject('Euromundo/checkout_page/checkbox_fare_breakdown'))

// ✅ Confirmación: checkbox "Importante (gastos adicionales)"

// Crear dinámicamente el TestObject con XPath robusto
TestObject gastosCheckbox = new TestObject()
gastosCheckbox.addProperty("xpath", ConditionType.EQUALS, "//input[@type='checkbox' and @name='checkBox1']")

// Obtener el elemento y verificar si está seleccionado
WebElement checkboxElement = WebUiCommonHelper.findWebElement(gastosCheckbox, 10)
boolean isSelected = checkboxElement.isSelected()

if (!isSelected) {
    WebUI.comment('⚠️ El checkbox de gastos adicionales NO está seleccionado. Procediendo a hacer clic.')
    WebUI.click(gastosCheckbox)
} else {
    WebUI.comment('🟢 El checkbox de gastos adicionales ya está seleccionado. Todo bien.')
}
...
// ✅ Confirmación: Términos y Condiciones
...
String textoCompleto = WebUI.getText(findTestObject('Euromundo/quotes/validation_quote')).trim()

WebUI.comment("📋 Texto capturado: [" + textoCompleto + "]")
