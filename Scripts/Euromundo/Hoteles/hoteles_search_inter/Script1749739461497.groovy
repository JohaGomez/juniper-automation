...
// ===========================
// Políticas de Cancelación - Bloque B (Comparación con A)
// ===========================

// 1. Click en el botón "Revise las políticas"
TestObject btnRevisePoliticas = new TestObject("btnRevisePoliticas")
btnRevisePoliticas.addProperty(
    "xpath",
    ConditionType.EQUALS,
    "//button[contains(@class,'confirm-booking__general-conditions') and contains(.,'Revise las políticas')]"
)

WebUI.waitForElementClickable(btnRevisePoliticas, 10)
WebUI.click(btnRevisePoliticas)
...
modalPoliticas_B.addProperty(
    "xpath",
    ConditionType.EQUALS,
    "//*[@id='main-modal']//div[@class='modal-body']"
)
WebUI.waitForElementVisible(modalPoliticas_B, 10)

WebDriver driver = DriverFactory.getWebDriver()

// 3. Capturar políticas B
List<Map<String, String>> politicas_B = []
List<WebElement> reglasDiv_B = driver.findElements(By.xpath("//*[@id='main-modal']//div[@class='row cancelation-rule']"))

if (reglasDiv_B.size() > 0) {
    for (WebElement regla : reglasDiv_B) {
        String condicion = regla.findElement(By.xpath(".//div[contains(@class,'rule-condition')]")).getText().trim()
        String cargo = "N/A"
        List<WebElement> cargos = regla.findElements(By.xpath(".//div[contains(@class,'rule-charge')]"))
        if (!cargos.isEmpty()) {
            cargo = cargos[0].getText().trim()
        }
        politicas_B.add([condicion: condicion, cargo: cargo])
    }
} else {
    // Caso alternativo: políticas en <p>
    TestObject politicasParrafo_B = new TestObject("politicasParrafo_B")
    politicasParrafo_B.addProperty(
        "xpath",
        ConditionType.EQUALS,
        "//*[@id='main-modal']//div[@class='modal-body']//p"
    )

    if (WebUI.verifyElementPresent(politicasParrafo_B, 5, FailureHandling.OPTIONAL)) {
        String textoPoliticas = WebUI.getText(politicasParrafo_B).trim()
        List<String> lineas = textoPoliticas.split("<br>|\\r?\\n").collect { it.trim() }
        for (String linea : lineas) {
            if (linea.startsWith("Cancelando")) {
                int idx = linea.lastIndexOf(":")
                String condicion = (idx > 0) ? linea.substring(0, idx).trim() : linea
                String cargo = (idx > 0) ? linea.substring(idx + 1).trim() : "N/A"
                politicas_B.add([condicion: condicion, cargo: cargo])
            }
        }
    }
...
// 4. Logs de B
for (int i = 0; i < politicas_B.size(); i++) {
    String msg = "📌 [B] Política ${i+1}: Condición='${politicas_B[i]['condicion']}' | Cargo='${politicas_B[i]['cargo']}'"
    println(msg)
...
if (politicas != null && politicas.size() > 0) {
    for (int i = 0; i < Math.max(politicas.size(), politicas_B.size()); i++) {
        def polA = (i < politicas.size()) ? politicas[i] : [condicion: "N/A", cargo: "N/A"]
        def polB = (i < politicas_B.size()) ? politicas_B[i] : [condicion: "N/A", cargo: "N/A"]

        if (polA == polB) {
            KeywordUtil.logInfo("✅ Política ${i+1} coincide entre A y B")
        } else {
            KeywordUtil.markWarning("❌ Diferencia en política ${i+1}: A='${polA}' vs B='${polB}'")
        }
    }
} else {
    KeywordUtil.markWarning("⚠️ No hay políticas A guardadas para comparar")
}


// 6. Cerrar modal (clic afuera si la X falla)
TestObject btnCustom = new TestObject("btnCustom")
btnCustom.addProperty("xpath", ConditionType.EQUALS, "//*[@id='confirm-booking']/div[1]/div/div/div[1]/button")

// Esperar y hacer clic
WebUI.waitForElementClickable(btnCustom, 10)
WebUI.click(btnCustom)
WebUI.comment("✅ Click realizado en el botón del confirm-booking")
...
// ==========================
// Código para abrir toggle y guardar la política de cancelación
// ==========================

// 0. Abrir toggle de detalles
TestObject toggleBtn = new TestObject("toggleBtn")
toggleBtn.addProperty("xpath", ConditionType.EQUALS, "//*[@class='shopping-basket__line-product-name']/button[contains(@class,'shopping-basket__line-product-extended-details-toggle')]")

if (WebUI.waitForElementClickable(toggleBtn, 5, FailureHandling.OPTIONAL)) {
	WebUI.click(toggleBtn)
}

// 1. Click en el enlace "Política de cancelación"
TestObject verPoliticaBtn2 = new TestObject("verPoliticaBtn2")
verPoliticaBtn2.addProperty("xpath", ConditionType.EQUALS, "//*[@id='main-content']//a[contains(text(),'Política de cancelación')]")

WebUI.click(verPoliticaBtn2)

// 2. Esperar a que se muestre el modal
TestObject modalPoliticas_C = new TestObject("modalPoliticas_C")
modalPoliticas_C.addProperty("xpath", ConditionType.EQUALS, "//*[@id='main-modal']")
WebUI.waitForElementVisible(modalPoliticas_C, 10)

// 3. Capturar bloque completo de políticas
TestObject politicasCompleto_C = new TestObject("politicasCompleto_C")
politicasCompleto_C.addProperty("xpath", ConditionType.EQUALS, "//*[@id='main-modal']//div[@class='modal-body']")
String politicasTexto_C = WebUI.getText(politicasCompleto_C).trim()

// 4. Logs
println "📌 [C] Texto completo en modal: ${politicasTexto_C}"

// 5. Validación básica
if (politicasTexto_C.contains("Cancelando desde")) {
	KeywordUtil.logInfo("✅ [C] Se encontró al menos una política con 'Cancelando desde'")
} else {
	KeywordUtil.markWarning("⚠️ [C] No se encontró ninguna política con 'Cancelando desde'")
}

// 6. Cerrar el modal
// Crear TestObject dinámico
TestObject btnBooking = new TestObject("btnBooking")
btnBooking.addProperty("xpath", ConditionType.EQUALS, "//*[@id='booking-details']/div[1]/div/div/div[1]/button")

// Esperar y hacer clic
WebUI.waitForElementClickable(btnBooking, 10)
WebUI.click(btnBooking)

WebUI.comment("✅ Click realizado en el botón de booking-details")
